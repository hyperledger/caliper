...
describe('When testing a Peer Gateway Wallet Facade Implementation', () => {
    before(() => {
        WalletFacadeFactory = require('../../../lib/connector-versions/peer-gateway/WalletFacadeFactory');
        WalletFacade = require('../../../lib/connector-versions/peer-gateway/WalletFacade');
    });
    ...
    it('A wallet facade should return a map', async () => {
        const walletFacade = await new WalletFacadeFactory().create();
        walletFacade.getWallet().should.be.instanceOf(Map);
    });
});
