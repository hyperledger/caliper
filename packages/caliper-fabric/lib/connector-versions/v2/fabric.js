...
const logger = CaliperUtils.getLogger('connectors/v2/fabric');
...
class Fabric extends BlockchainConnector {
    ...
    async _initializeAdmins(workerInit) {
        ...
        for (const org of orgs) {
            ...
            // Set the admin explicitly based on its crypto materials in the connection profile
    ...
    async _instantiateContracts() {
        ...
        for (const channel of channels) {
            ...
            for (const contractInfo of contractInfos) {
                ...
                CaliperUtils.assertProperty(queryResponse, 'queryResponse', 'contracts');
    ...
    async _prepareClientProfile(org, clientName, profileName) {
        ...
        const profile = await FabricClient.loadFromConfig(this.networkUtil.getNewNetworkObject());
