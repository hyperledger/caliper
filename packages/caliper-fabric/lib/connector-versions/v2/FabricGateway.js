...
class V2FabricGateway extends ConnectorBase {
    ...
    async getContext(roundIndex, args) {
        if (!this.context) {
            ...
            await this._buildPeerCache();  // can't this be done once ?
    ...
    async _sendSingleRequest(request) {
        ...
        return this._submitOrEvaluateTransaction(request, request.readOnly === undefined || !request.readOnly);
    }
    ...
    async _prepareGatewayAndContractMapsForEachIdentity() {
        ...
        for (const organization of this.connectorConfiguration.getOrganizations()) {
            ...
            for (const aliasName of aliasNames) {
                const wallet = this.connectorConfiguration.getWalletForAliasName(aliasName);
                const gateway = await this._createGatewayWithIdentity(organization, aliasName, wallet);
