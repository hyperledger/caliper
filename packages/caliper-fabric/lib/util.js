...
const commLogger = CaliperUtils.getLogger('util.js');
...
module.exports.setupChaincodeDeploy = function() {
    if (typeof process.env.OVERWRITE_GOPATH === 'undefined' ||
        ...
        process.env.GOPATH = CaliperUtils.resolvePath('.', networkRoot);
    }
};
...
module.exports.init = function(config_path, root) {
...
async function getAdmin(client, userOrg) {
    try {
        ...
        let keyPEM, certPEM;
        if(org.user) {
            keyPEM = fs.readFileSync(CaliperUtils.resolvePath(org.user.key));
            certPEM = fs.readFileSync(CaliperUtils.resolvePath(org.user.cert));
        }
        else {
...
async function getOrdererAdmin(client) {
    try {
        ...
        let keyPEM, certPEM;
        if(orderer.user) {
            keyPEM = fs.readFileSync(CaliperUtils.resolvePath(orderer.user.key));
            certPEM = fs.readFileSync(CaliperUtils.resolvePath(orderer.user.cert));
        }
        else {
