...
namespace _ {
	...
	export function readdir(dir: string, configFiles: Array<string>): Promise<string[]> {
		configFiles = configFiles || [];
		var files = fs.readdirSync(dir);
		for (var i in files) {
			var configFilePath = dir + "/" + files[i];
			if (fs.statSync(configFilePath).isDirectory()) {
				readdir(configFilePath, configFiles);
			} else {
				if (configFilePath.includes("calrun.yaml")){
					configFiles.push(configFilePath);
				}
			}
		}
        return Promise.resolve(configFiles);
	}
...
export class ConfigFileProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	async _readDirectory(uri: vscode.Uri): Promise<[string, vscode.FileType][]> {
		...
		for (let i = 0; i < children.length; i++) {
			...
			const stat = await this._stat(path.join(child));
	...
	async getChildren(element?: Entry): Promise<Entry[]> {
		...
		if (workspaceFolder) {
			...
			return children.map(([name, type]) => ({ uri: vscode.Uri.file(path.join(name)), type }));
		}
