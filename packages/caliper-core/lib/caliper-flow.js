...
const bftsmartUtils = require('./../../caliper-bftsmart/lib/bftsmartUtils');
const cordaUtils = require('./../../caliper-corda/lib/cordaUtils');
...
module.exports.run = async function(absConfigFile, absNetworkFile, admin, clientFactory, workspace) {
    ...
    const cordaUtil = new cordaUtils(adminClient, workspace);
    const bftsmartUtil = new bftsmartUtils(adminClient, workspace);
    ...
    try {
        ...
        if (networkObject.caliper.blockchain === 'corda') {
            let args = configObject.test.rounds;
            let results = await cordaUtil.computeRounds(args);
            await cordaUtil.saveResults(JSON.stringify(results), configObject.test.result_filename);
            successes = results.length; failures = args.length - successes;
        } else if (networkObject.caliper.blockchain === 'bftsmart') {
            let nb = parseInt(networkObject.caliper.servers.number);
            let sclass = networkObject.caliper.servers.class;
            if (networkObject.caliper.servers.init) {
                for (let i = 0; i < nb; i++) {
                    await adminClient.startServer(i, sclass);
                }
            }
            let test = configObject.test;
            let results = await bftsmartUtil.computeRounds(test);
            await bftsmartUtils.saveResults(JSON.stringify(results), configObject.test.result_filename);
            successes = results.length; failures = test.rounds.length - successes;
        } else {
        ...
        await cordaUtils.stopGradleDaemon();
