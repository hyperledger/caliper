 ...
 * @return {Promise<SawtoothClient>} The initialized adapter instance.
 * @async
 */
async function initHandler(context, message) {
    return new SawtoothClient(context.networkConfigPath, context.workspacePath, context.workerId);
}
...
async function main (){
    ...
    const messenger = new Messenger({type, sut: 'sawtooth'});
