 ...
 * @return {Promise<IrohaClient>} The initialized adapter instance.
 * @async
 */
async function initHandler(context, message) {
    return new IrohaClient(context.networkConfigPath, context.workspacePath, context.workerId);
}
...
async function main (){
    ...
    const messenger = new Messenger({type, sut: 'iroha'});
