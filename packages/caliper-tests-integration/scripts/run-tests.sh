...
if [[ "${BENCHMARK}" == "composer" ]]; then
    ...
    caliper benchmark run -c benchmark/simple/config.yaml -n network/fabric-v1.4.1/2org1peercouchdb/fabric-node.yaml -w ../caliper-samples/ --caliper-fabricccp-usegateway
    rc=$?
    if [[ $rc != 0 ]]; then
        exit $rc;
    else
        # Run with channel creation using an existing tx file in LevelDB, using a low level Caliper client
        caliper benchmark run -c benchmark/simple/config.yaml -n network/fabric-v1.4/2org1peergoleveldb/fabric-go.yaml -w ../caliper-samples/
        rc=$?
        exit $rc;
    fi
else
