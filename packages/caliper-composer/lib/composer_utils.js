 ...
 * @param {Object} config Configuration data in Json format
 * @param {String} workspacepath the absolute path to the root worksapce
 * @returns {Object} the common connection profile
 */
function createCommonConnectionProfile(orgName, config, workspacepath) {
    let profile = {};
    ...
    Object.keys(config.composer.network.orderers).forEach((key) => {
        ...
        if(config.composer.network.tls) {
            ...
            let tlsCACerts = {};
            let certPath = CaliperUtils.resolvePath(configOrderer.cert, workspacepath);
    ...
    Object.keys(config.composer.network.peers).forEach((key) => {
        ...
        if(config.composer.network.tls) {
            ...
            let tlsCACerts = {};
            let certPath =  CaliperUtils.resolvePath(configPeer.cert, workspacepath);
            tlsCACerts.path = certPath;
 ...
 * @param {Object} config Configuration data in Json format
 * @param {string} workspacepath The absolute path to the root location for the application configuration files.
 */
async function createAdminBusNetCards(config, workspacepath) {
    let orgs = config.composer.network.organizations;
 ...
 * @param {String} workspacepath the absolute path to workspace root
 ...
 */
async function runtimeInstall(businessNetwork, workspacepath, installOptions, cardName) {
    let adminConnection = new AdminConnection();
    let businessNetworkDefinition;

    //check the file is there
    let filePath = path.join(CaliperUtils.resolvePath(businessNetwork.path, workspacepath), businessNetwork.id);
