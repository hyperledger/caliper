...
class Composer extends BlockchainInterface {
    ...
    /**
     * Initializes the Composer adapter.
    ...
    init() {
        ...
        return this.initialiseFabric(config)
            .then(() => {
            // Create id cards
                return composer_utils.createAdminBusNetCards(config, this.workspaceRoot);
            })
            .catch((err) => {
    ...
    installSmartContract() {
        ...
        return busnets.reduce((promiseChain, busnet) => {
            return promiseChain.then(() => {
                return composer_utils.runtimeInstall(busnet, this.workspaceRoot, null, 'PerfPeerAdmin@' + busnet.org);
            });
        }, Promise.resolve())
            .then((result) => {
