    ...
    installSmartContract() {
        ...
        .then((result) => {
            ...
            return chaincodes.reduce((promiseChain, busnet) => {
                return promiseChain.then(() => {
                    console.log('starting runtime for ' + busnet.id + ' using card PerfPeerAdmin@' + busnet.orgs[0] + ' ......');
                    return composer_utils.networkStart(busnet.path + '/' + busnet.id + '.bna', 'PerfPeerAdmin@' + busnet.orgs[0], busnet.loglevel);
                });
            }, Promise.resolve()) 
        })
        .then(() => {
