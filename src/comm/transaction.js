...
class TxStatus {
     ...
     * @param {string} status, status status
     * @param {date} time_create, transaction create time
     * @param {date} time_final, transaction final time
     * @param {bytes} result, transaction result
    ...
    constructor(id, status, time_create, time_final, result, verified, flags, error_messages) {
        if (arguments.length === 1){
            ...
            this.status = {
                id: id,
                status: status,  // submitting status, three status 'created', 'success', 'failed' are reserved
